...
/* Modified by Mykolas Bamberg, 2025 */
/* Added bar_graph function and modified icelink_flash_write_sectors */
...
void bar_graph(int32_t k, int32_t n, const char* title, FILE* stream)
{
    fprintf(stream, "\r\033[K");
    fprintf(stream, "%s %3d%% [ ", title, k * 100 / n);
    for (size_t i = 0; i < BAR_GRAPH_WIDTH; i++) {
        fputc(i < k * BAR_GRAPH_WIDTH / n ? '=' : ' ', stream);
    }
    fprintf(stream, " ]");
    fflush(stream);
}
...
int icelink_flash_write_sectors(uint32_t flash_addr, uint32_t sector_num, uint8_t *buf)
{
    ...
    for(i = 0; i < sector_num; i++) {
        faddr = flash_addr + i * SPI_FLASH_SECTOR_SIZE;
        bar_graph(i, sector_num, "write", stdout);
        icelink_flash_write_sector(faddr, &buf[i * SPI_FLASH_SECTOR_SIZE]);
    }
    bar_graph(1, 1, "write", stdout);
    printf("\n");
